...
public class MediaController : ControllerBase
{
    ...
    [HttpPost]
    public async Task<ActionResult<object>> Upload(IFormFile file, [FromQuery] string? folder = "cases")
    {
        if (file is null || file.Length == 0) return BadRequest("No file.");
        var key = $"{folder?.Trim('/')}/{DateTime.UtcNow:yyyy/MM}/{Guid.NewGuid()}{Path.GetExtension(file.FileName)}";
        await using var s = file.OpenReadStream();
        var url = await _storage.UploadAsync(key, s, file.ContentType, file.Length); // length passed here
